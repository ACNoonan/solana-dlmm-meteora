//! Bin id ↔ price (Q64.64) conversions.
//!
//! Extracted from MeteoraAg/dlmm-sdk:
//! - `commons/src/math/u64x64_math.rs` (`pow`, `SCALE_OFFSET`, `ONE`, `MAX_EXPONENTIAL`)
//! - `commons/src/math/price_math.rs` (`get_price_from_id`)
//! - `commons/src/constants.rs` (`MIN_BIN_ID`, `MAX_BIN_ID`, `BASIS_POINT_MAX`)
//!
//! The arithmetic is byte-for-byte identical to upstream. The only
//! adjustments are dropping `anyhow::Result` / `.context("overflow")` in
//! favor of our internal [`ErrorCode`].
...
// --- commons/src/math/price_math.rs ---

/// Q64.64 price of one bin given an `active_id` and `bin_step` (in bps).
///
/// `price = (1 + bin_step / 10_000) ^ active_id`, computed as integer math
/// in Q64.64. The shape mirrors upstream byte-for-byte; the only change is
/// that the upstream `anyhow::Result` + `.context("overflow")` calls are
/// replaced by our internal `Result<u128>` and `ErrorCode::MathOverflow`.
pub fn get_price_from_id(active_id: i32, bin_step: u16) -> Result<u128> {
    ...
    let base = ONE.checked_add(bps).ok_or(ErrorCode::MathOverflow)?;

    pow(base, active_id).ok_or(ErrorCode::MathOverflow)
}
