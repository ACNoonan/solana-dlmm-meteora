...
**Pre-v0.1 (milestones 1+2+3 of 7 landed).** Crate scaffold is in place
and three layers of integer math have been ported verbatim from
`MeteoraAg/dlmm-sdk` `commons/`: bin id → Q64.64 price, the 256-bit
multiply-divide engine, and the structural per-bin swap primitives
(`get_amount_in`/`_out`, `get_max_amount_in`/`_out`). 11 value-pinned
tests, all expectations generated by an independent Python oracle.
